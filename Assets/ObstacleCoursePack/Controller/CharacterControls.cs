...
public class CharacterControls : MonoBehaviour {
	...
	public float gravity = 10.0f; //it's a acceleration
	...
	void FixedUpdate () {
		if (canMove) //If player is not hit
		{
			if (moveDir.x != 0 || moveDir.z != 0)
			{
				...
				Quaternion tr = Quaternion.LookRotation(targetDir); //Rotation of the character to where it moves		Final target rotation
			...
			else
			{
				if (!slide) 
				{
					...
					velocityChange.z = Mathf.Clamp(velocityChange.z, -maxVelocityChange, maxVelocityChange);
					rb.AddForce(velocityChange, ForceMode.VelocityChange);
					if (velocity.y < -maxFallSpeed)  //The falling speed can't surplus than maxFallApeed
						rb.velocity = new Vector3(velocity.x, -maxFallSpeed, velocity.z);
				}
				else if (Mathf.Abs(rb.velocity.magnitude) < speed * 1.0f) //If it's nearly reached the slide ground when falling
				{
					rb.AddForce(moveDir * 0.15f, ForceMode.VelocityChange);
				}
			}
		}
		else //If player is hit
		{
	...
	private void Update()
	{
		...
		RaycastHit hit;
		if (Physics.Raycast(transform.position, -Vector3.up, out hit, distToGround + 0.1f)) //The position of out in C# doesn't matter
		{
			if (hit.transform.tag == "Slide") //If the collision target below with the tag of "Slide" 
			{
				slide = true;
			}
			else
